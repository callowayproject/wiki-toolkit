"""Unit tests for wiki_toolkit.wiki's internal (non-Click) logic."""
...
def test_build_catalog_status_proposed_when_any_source_proposed(
    ...
    assert result.entries[0].status == "proposed"
...
def test_build_catalog_flags_malformed_frontmatter(make_docs_tree: Callable[[], Path]) -> None:
    ...
    assert result.violations[0].path == "docs/wiki/bad.md"
