...
**lint** — unchanged: contradictions, staleness, orphan pages, missing cross-references. Also flags a `relationships:` entry whose `target` doesn't resolve to an existing page — same resolution rule as missing-cross-reference detection already uses for body `[[wikilinks]]` — and flags drift between a page's `confidence:` block and its recomputed marker counts. Flagged, not rejected: lint reports, it doesn't block; the write gate's PR review is the enforcement point.
